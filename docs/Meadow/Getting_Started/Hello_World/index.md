...
* `Meadow.Foundation` - [Meadow.Foundation](/Meadow/Meadow.Foundation/index) is a set of open-source peripheral drivers and hardware control libraries that make hardware development with Meadow, plug-and-play.
...
The device class defines properties and capabilities of the current device, such as the pins. While your app is running, your code can access the current device from the `Device` property on the [`Resolver`](/docs/api/Meadow.Contracts/Meadow/Resolver/) class, allowing you to access them using autocomplete, via the specific device type:
