...
You can then specify which port to use via for future commands with the `config route` command. Then, after configuring which port to use, future commands will be directed to that port.

```bash
meadow config route /dev/tty.usbmodem336F336D30361
```
